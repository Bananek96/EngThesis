...
class Verification:
    ...
    def valid_proof(transfers, last_hash, proof):
        ...
        Arguments:
            transfers: The transfers of the block for which the proof is created.
    ...
    def verify_transfer(transfer, get_balance, check_funds=True):
        """Verify a transfer by checking whether the sender has sufficient files.
        ...
        if check_funds:
            ...
            return sender_balance >= transfer.file and Wallet.verify_transfer(transfer)
        else:
            return Wallet.verify_transfer(transfer)

    @classmethod
    def verify_transfers(cls, open_transfers, get_balance):
        """Verifies all open transfers."""
